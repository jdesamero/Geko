...
class Geko_String_Highlight
{
	...
	public function highlightPhrase( $sPhrase, $mWords, $sStartHighlight = NULL, $sEndHighlight = NULL ) {
		...
		foreach ( $aWords as $sWord ) {
			$sPhrase = preg_replace( sprintf( '/%s/si', $sWord ), sprintf( '%s\0%s', $sStartHighlight, $sEndHighlight ), $sPhrase );
		}
		...
		if ( $iPhraseLen > $iTruncateLen ) {
			...
			if ( $iFirstKwPos > $iPaddingLen ) {
				...
				if ( ( $iPhraseLen - $iStartPos ) > $iTruncateLen ) {
					// trim both ends
					$sFmt = sprintf( '%s%s%s', $this->sEllipsis, $this->trimEnd( $this->trimStart( substr( $sPhrase, $iStartPos, $iTruncateLen ) ) ), $this->sEllipsis );
				} else {
					// trim the start part
					$sFmt = sprintf( '%s%s', $this->sEllipsis, $this->trimStart( substr( $sPhrase, $iPhraseLen - $iTruncateLen ) ) );
				}
				
			} else {
				// trim the end part
				$sFmt = sprintf( '%s%s', $this->trimEnd( substr( $sPhrase, 0, $iTruncateLen ) ), $this->sEllipsis );
			}
