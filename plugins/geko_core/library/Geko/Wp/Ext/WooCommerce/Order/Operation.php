...
class Geko_Wp_Ext_WooCommerce_Order_Operation extends Geko_Wp_Options_Operation
{
	
	protected $_sTitle = 'Order Export';
	...
	protected function preWrapDiv() {
		...
		$oAggOrders = $this->oneExt_WooCommerce_Order_Query( array(
			'aggregate_mode' => TRUE
		), FALSE );
		
		$aJsonParams = array(
			'date' => array(
				'min' => Geko_Wp_Date::formatDateForRange( $oAggOrders->getTransactionMinDate() ),
				'max' => Geko_Wp_Date::formatDateForRange( $oAggOrders->getTransactionMaxDate() )
			)
		);
	...
	//
	protected function formFieldsExportOrders() {
		...
		?>
		<p>Export order information (as .xls file).</p>
		<p>
	...
	//
	public function doActionExportOrders( $aOperation ) {
		...
		if ( $bContinue ) {
			...
			$oExcel = new Geko_Wp_Ext_WooCommerce_Order_Export();
