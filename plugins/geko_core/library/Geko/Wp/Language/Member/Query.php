	...
	public function modifyQuery( $oQuery, $aParams )
	{
		...
		if (
			...
			$oSubQuery
				...
				->from( $wpdb->geko_lang_group_members, 'lgm1' )
				
				->joinLeft( $wpdb->geko_lang_groups, 'lg1' )
		...
		if ( $aParams['add_siblings_field'] ) {
			...
			$oSubQuery
				...
				->from( $wpdb->geko_lang_group_members, 'm1' )
		...
		if ( is_array( $aParams['filter'] ) ) {
			...
			foreach ( $aParams['filter'] as $aFilter ) {
				...
				$aWhere[] = ' (' . implode( ') AND (', $aSubWhere ) . ') ';
			}
			
			$oQuery->where( ' (' . implode( ') OR (', $aWhere ) . ') ' );
		}
