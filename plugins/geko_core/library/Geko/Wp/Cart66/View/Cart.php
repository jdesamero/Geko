...
$fPointsDiscount = -0.41;

$subtotal = $subtotal + $fPointsDiscount;
...
if($taxData > 0) {
  ...
  $oTaxRate->loadByState( 'All Sales' );
  
  $tax = number_format($subtotal * ($oTaxRate->rate/100), 2, '.', '');
  
}


$fGrandTotal = $subtotal + $tax + $shipping;
...
<tr class="points_discount">
	<td colspan="2">&nbsp;</td>
	<td class="alignRight strong"><?php _e( 'Points Discount' , 'cart66' ); ?>:</td>
	<td class="strong cart66-align-right"><?php echo Cart66Common::currency( $fPointsDiscount ); ?></td>
</tr>
      ...
      <tr class="total">
        ...
        <td class="strong grand-total-amount cart66-align-right">
          <?php 
            ...
            echo Cart66Common::currency( $fGrandTotal );
          ?>
        </td>
      </tr>
      </tbody>
